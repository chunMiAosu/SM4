...
public class SM4_4
{
    ...
    /*    byte[] plaintext = {(byte) 0x01, (byte) 0x23, (byte) 0x45, (byte) 0x67, (byte) 0x89, (byte) 0xAB, (byte) 0xCD
        ...
        if (fileSize < 16)
        {
            len = (int)(fileLength + 16-(fileLength%16)); //确保是16b即128bit的倍数，以便数据填充
        }
        else
        {
            len = 16 << 20; //16MB
        ...
        while ((len = in.read(plaintext)) != -1)
        {
            ...
            {
                byte[] tmp_in = Arrays.copyOfRange(plaintext,i,i+16);
        ...
        File writeFile = writeFile = new File("./timeData_4.csv");;
